...
namespace json_utils 
{
	...
	template <typename T>
	T getStruct(const nlohmann::json& config, const std::string& key, const T& defaultValue)
	{
		if (config.find(key) != config.end())
		{
	...
	struct JsonUtils
	{
		static std::vector<T> getVector(const json& config, const std::string& key)
		{
			...
			if (config.find(key) != config.end())
			{
				std::for_each(std::begin(config.at(key)), std::end(config.at(key)), 
								[&values](auto&& item) { values.emplace_back(T(item)); });
			}
			return values;
		}

		static std::list<T> getList(const nlohmann::json& config, const std::string& key)
		{
			std::list<T> values;
			if (config.find(key) != config.end())
			{
		...
		static std::map<std::string, T> getMap(const nlohmann::json& config, const std::string& key)
		{
	...
	{
		static std::vector<std::string> getVector(const nlohmann::json& config, const std::string& key)
		{
