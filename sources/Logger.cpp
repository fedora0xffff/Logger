...
logger::Logger::Logger()
	...
	if (!logger_thread_running) {
		...
		logger_thread = std::thread([&]() { this->startWriteLoop(); });
	}
}
...
std::string logger::Logger::getLoggerFile() {
	...
	try {
		if (fs::is_directory(path) || fs::create_directory(path)) {
			os << path << '/' << config.getLogFileName() << getFileTimeStamp()
			   << ".log";
		...
		std::cout << "Failed to create the log file, restart as sudo"
				  << std::endl;
	}
...
void logger::Logger::writeLog() {
	...
	while (!message_queue.empty()) {
		...
		if (config.getLogToStdout()) {
			std::cout << getEntryTimeStamp() << ": " << message_queue.front()
					  << std::endl;
		}
		message_queue.pop();
	}
